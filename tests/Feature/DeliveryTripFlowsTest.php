...
class DeliveryTripFlowsTest extends TestCase
{
    ...
    public function test_user_can_create_delivery_trip_log_with_team_and_costs(): void
    {
        $user = User::factory()->create(['role' => 'user']);
        $teamA = User::factory()->create(['name' => 'Andi Team', 'role' => 'user']);
        $teamB = User::factory()->create(['name' => 'Budi Team', 'role' => 'user']);
        ...
        $response = $this->actingAs($user)->post(route('delivery-trips.store'), [
            ...
            'member_user_ids' => [$teamA->id, $teamB->id],
            'extra_member_names' => "Gudang 1\nGudang 2",
            'fuel_cost' => 120000,
            'toll_cost' => 50000,
            'meal_cost' => 30000,
            'other_cost' => 10000,
        ...
        $this->assertSame(4, (int) $trip?->member_count);

        $this->assertDatabaseHas('delivery_trip_members', [
            'delivery_trip_id' => (int) $trip?->id,
            'member_name' => 'Andi Team',
        ]);
        $this->assertDatabaseHas('delivery_trip_members', [
            'delivery_trip_id' => (int) $trip?->id,
            'member_name' => 'Gudang 1',
        ]);
    ...
    {
        $admin = User::factory()->create(['role' => 'admin', 'permissions' => ['*']]);
        $member = User::factory()->create(['name' => 'Member Satu', 'role' => 'user']);
