    ...
    <form method="post" action="{{ route('delivery-notes.store') }}">
        ...
        <div class="card">
            <div class="form-section">
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <label>{{ __('txn.recipient_name') }} <span class="label-required">*</span></label>
        ...
        <div class="card">
            ...
            <table id="items-table" style="margin-top: 12px;">
                <thead>
                <tr>
                    <th style="width: 25%">{{ __('txn.product') }} ({{ __('txn.optional') }})</th>
                    <th>{{ __('txn.code') }} ({{ __('txn.optional') }})</th>
                    <th>{{ __('txn.name') }} *</th>
                    <th>{{ __('txn.unit') }}</th>
                    <th>{{ __('txn.qty') }} *</th>
                    <th>{{ __('txn.price') }} ({{ __('txn.optional') }})</th>
                    <th>{{ __('txn.notes') }}</th>
                    <th></th>
                </tr>
                </thead>
    ...
    <script>
        ...
        function addRow() {
            ...
            tr.innerHTML = `
                ...
                <td><input name="items[${index}][unit]" class="unit"></td>
                <td><input name="items[${index}][quantity]" type="number" min="1" value="1" required></td>
                <td><input name="items[${index}][unit_price]" type="number" min="0" step="0.01"></td>
