...
@section('title', __('txn.create_delivery_note_title').' - '.config('app.name', 'Laravel'))
    ...
    <script>
        ...
        function createProductAutocomplete(inputEl, hiddenEl, onSelect) {
            ...
            function open(matches) {
                ...
                dropdown.innerHTML = matches.length === 0
                    ...
                    : matches.map((p, i) => {
                        ...
                        return `<div class="product-ac-item${outOfStock?' out-of-stock':''}" data-idx="${i}"><div><div class="product-ac-name">${esc(p.name)}</div>${p.code?`<div class="product-ac-code">${esc(p.code)}</div>`:''}</div><div class="product-ac-meta">Stok: ${p.stock??'?'} ${esc(unit)}</div></div>`;
                    }).join('');
                document.body.appendChild(dropdown); position();
                dropdown.addEventListener('mousedown', e => { const item = e.target.closest('.product-ac-item'); if (!item) return; e.preventDefault(); pick(parseInt(item.dataset.idx,10)); inputEl.closest('tr')?.querySelector('.qty-input,.qty')?.focus(); });
            ...
            inputEl.addEventListener('keydown', e => {
                ...
                else if (e.key === 'Enter' && dropdown && activeIdx >= 0) { e.preventDefault(); pick(activeIdx); inputEl.closest('tr')?.querySelector('.qty-input,.qty')?.focus(); }
        ...
        function addRow(prefill = null) {
            ...
            tr.innerHTML = `
                <td>
                    <input type="text" class="product-search" name="items[${index}][product_name]" placeholder="Pilih barang terdaftar" autocomplete="off" value="${escapeAttribute(productText)}" required>
            ...
            createProductAutocomplete(
                ...
                (product) => {
                    setRowUnit(tr, product ? product.unit || '' : '');
                    if (product) { setProductFieldError(tr, ''); suggestOrderNoteForProduct(product.id, tr); }
                }
            );
            tr.querySelector('.remove').addEventListener('click', () => { tr.remove(); recalcItemsTotal(); });
            recalcItemsTotal();
        }
